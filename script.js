...
function sma_star(start, goal, max_depth = 1000, max_open_size = 40) {
  ...
  while (open_set.length > 0) {
    // Sort open set by f value (ascending)
    ...
    for (var [neighbor, weight] of Graph_nodes[current.name]) {
      ...
      // Check if the neighbor is already in the open set with a higher f value
      var existingNodeIndex = open_set.findIndex(
        (node) => node.name === neighbor && node.f > f
      );
      if (existingNodeIndex !== -1) {
        open_set.splice(existingNodeIndex, 1); // Remove the existing node with higher f value
      }

      if (open_set.length >= max_open_size) {
        // Remove node with highest f value
    ...
    if (startNode && endNode) {
      ...
      if (i === 0) {
        routeDetails.push({
          id: startNode.id,
          name: startNode.name,
          koridor: startNode.koridor.join(", "),
        });
      }
